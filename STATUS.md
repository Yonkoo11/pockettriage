...
|---|---|---|
| "Runs on Gemma 4 E2B / E4B" | Only **E2B** has been run against the eval. **2026-05-24 update — verification hit a hardware ceiling, not a software one.** Pull eventually succeeded (10.5 GB, ~28 min, after one earlier R2 CDN timeout). Model loads into Ollama. But this laptop is an 8 GB MacBook Air; the E4B weight file is 10.5 GB and Ollama can only fit 3.26 GB in VRAM, swapping the rest to disk. A raw `/api/generate` call requesting a 5-token reply returned zero bytes after 240 s — the model cannot complete a single token before timing out. Eval runner also killed at 5 min per scenario. Conclusion: **E4B is not eval-runnable on this hardware. Architecture parity with E2B remains untested.** | Need a 16+ GB Mac, a Linux box with ≥ 12 GB free RAM, or a one-off cloud GPU run (Modal / Replicate / HF Inference) to capture the 4-scenario numbers. Plain `eval_runner.py` invocation works on the target machine; just set `POCKETTRIAGE_OLLAMA_TAG=gemma4:e4b`. |
